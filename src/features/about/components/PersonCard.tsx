  ...
  return (
    <div className="flex flex-col gap-4">
      <button
        ...
        disabled={!canExpand}
        className={cn(
          'notch-corner-sm group relative aspect-square w-full overflow-hidden bg-[#1a1a1a] text-left',
      ...
      >
        {expanded && canExpand ? (
          <motion.div
            ...
            className="absolute inset-0 overflow-y-auto bg-[#052424] p-4 text-sm leading-relaxed text-white/80"
          >
            {bio}
          </motion.div>
        ) : (
          ...
          >
            ...
            <span className="pointer-events-none absolute inset-0 flex items-center justify-center bg-black/0 opacity-0 transition-all duration-300 group-hover:bg-black/20 group-hover:opacity-100">
              <span className="rounded-lg bg-white px-8 py-3 text-[11px] font-semibold tracking-wide text-[#052424] uppercase">
                Learn More
              </span>
            </span>
          </motion.div>
      ...
      <div>
        <p className="text-xl font-semibold text-[#052424]">{name}</p>

...
import {
  ...
  MOUNT_GRACE_MS,
  REDUCED_EXIT_MS,
  REDUCED_MARK_HOLD_END_MS,
  entryVariables,
} from './timeline';
import styles from './preloader.module.css';

/**
 * The entry, in two acts: a setting-out drawing assembling itself around the
 * PEN mark, then the mark handing that centre to the statement — three beats of
 * copy written a word at a time. It closes by retracting to a single line.
 *
 * WHAT THIS COMPONENT ACTUALLY DOES. None of act one. The field, the mark's
 * mask and the scan line are CSS keyframes on server-rendered markup, so they
 * begin in the frame the panel is painted rather than whenever the bundle
 * finishes arriving — a distinction worth a second or more on this app, which
 * ships R3F and framer before it can hydrate, and the reason the drawing is now
 * what fills that gap instead of a still screen. Act two needs React, because a
 * sentence written a word at a time is a schedule, not a keyframe. See
 * timeline.ts for the whole shape.
 *
...
/**
 * `mark` is the whole of act one — draw, reveal, hold — because React does not
 * need to distinguish its parts; it only needs to know when it ends.
 */
type Phase = 'mark' | 'statement' | 'exiting' | 'gone';
 ...
 * hydration — which on this bundle can be seconds later. Without this the mark
 * would hold for "hydration plus 1.9s" and the published timing would be
 * fiction; worse, the wait would be longest on exactly the slow connections
 * that already waited.
 *
 ...
 * just started", which errs towards showing the mark for too long rather than
 * cutting it off — and `MOUNT_GRACE_MS` catches the other direction.
 */
...
export function PENPreloader() {
  ...
  useEffect(() => {
    ...
    const holdEnd = reducedMotion ? REDUCED_MARK_HOLD_END_MS : MARK_HOLD_END_MS;
  ...
  return (
    <div
      ...
      /* Chrome over a page that is already mounted and reachable. Not
         `aria-hidden`: the drawing and the mark are decoration and mark
         themselves so, but the statement is the one thing here with something
         to say, and it carries its own readable sentence. */
      role="presentation"
    >
      <motion.div
      ...
      >
        ...
        <PreloaderMark clearing={past} />

        {/* Mounted only once the centre is free. Nothing fades it in: each beat
            writes its own words, and a wrapper fade on top of that would blunt
